...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'natorisana-voice:Kyopro': () => this.play(this.vKyopro)
    }));
    
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'natorisana-voice:Homete': () => this.play(this.vErai)
    }));
  ...
  loadVoice(){
    ...
    //競プロ
    this.getFile('kyopro',(buf)=>{this.vKyopro = buf;});
    
    //プログラミングできてえらい
    this.getFile('erai',(buf)=>{this.vErai = buf;});
